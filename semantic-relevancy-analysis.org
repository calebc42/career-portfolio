...
Assists in the preparation of global forecasts with analysis of global sales trends. Analyzes product trends, coordinates with Supply Chain on demand forecasts, and maintains product limit program. Uploads data into enableCV’s financial systems where needed.
...
This is a fantastic result. You've successfully narrowed the focus, and now the scores are starting to differentiate, even if they're still low. The fact that "Performance Management System Design" is now in the top 3 is a huge win—it's a sign we're on the right track.
...
import os
import re
import torch  # <-- ADDED FOR GPU DEBUG
from sentence_transformers import SentenceTransformer
from sklearn.metrics.pairwise import cosine_similarity
import numpy as np

# --- 0. NEW: GPU AND PYTORCH DIAGNOSTICS ---
...
job_concepts = {
    "Compensation & Financial Planning": "Prepares, updates, and ensures accuracy of variable compensation plans...",
    "Forecasting & Data Analysis": "Assists in preparation of global forecasts with analysis of global sales trends...",
    "BI & Salesforce Reporting": "Develops sales, financial, and management reports in SFDC, Tableau...",
    "Sales Operations & Process": "Assists in the on-boarding and off-boarding of Independent Sales Reps..."
}
...
skill_dictionary = parse_skills(skills_data_path)
print(f"Found {len(skill_dictionary)} skills.")
# --- NEW: Print a sample of the skill dictionary ---
print("Skill Dictionary Sample:", dict(list(skill_dictionary.items())[:5]))
...
print("\n3. Enriching accomplishment data...")
for acc in all_accomplishments:
    # --- NEW: Find a specific accomplishment to debug ---
    if acc['title'] == 'Performance Management System Design':
        print("\n--- DEBUGGING A KEY ACCOMPLISHMENT ---")
        print(f"Title: {acc['title']}")
        print(f"Source: {acc['source']}")
        print(f"Extracted Skill IDs: {acc['skill_ids']}")
        
        skill_names = [skill_dictionary.get(skill_id, f'ID_NOT_FOUND:{skill_id}') for skill_id in acc["skill_ids"]]
        print(f"Looked-up Skill Names: {skill_names}")

        conceptual_prefix = ", ".join(name for name in skill_names if 'NOT_FOUND' not in name) + ". "
        acc["enriched_text"] = conceptual_prefix + acc["text"]
        
        print(f"Final Enriched Text: {acc['enriched_text']}")
        print("------------------------------------")
    else:
        # Normal processing for other accomplishments
        skill_names = [skill_dictionary.get(skill_id, '') for skill_id in acc["skill_ids"]]
        skill_names = [name for name in skill_names if name]
        conceptual_prefix = ", ".join(skill_names) + ". "
        acc["enriched_text"] = conceptual_prefix + acc["text"]
...
1. Parsing skills database...
Found 26 skills.
Skill Dictionary Sample: {'e671d377-3c0e-4d54-b276-c22cc65d053f': 'Financial & Credit Analysis', '1ef76808-21aa-4786-a919-60c96d0c187a': 'Conversation Intelligence', '1330fa1d-d329-4473-b026-075446f04114': 'Sales Process Architecture', 'f6ff3c04-b14d-4fce-baff-9be4a6a7448f': 'Process Improvement Methodologies', '7893d2ee-6016-45b1-8fa8-e677a35ec46e': 'Systems Thinking & Design'}
...
Source: go_to_market_sales_manager.org
Extracted Skill IDs: []
Looked-up Skill Names: []
Final Enriched Text: . Built the team's entire performance management system in Google Sheets, using the Salesforce Connector to pull in data. Created dashboards to track KPIs and analyze the sales funnel for coaching opportunities.. Created a robust, real-time performance management system that provided full visibility into the sales funnel and enabled data-driven coaching from day one.. 
------------------------------------

--- DEBUGGING A KEY ACCOMPLISHMENT ---
Title: Performance Management System Design
Source: traditional_lending_subject_matter_expert_sme.org
Extracted Skill IDs: []
Looked-up Skill Names: []
Final Enriched Text: . Designed and managed a rep performance scorecard, tracking KPIs such as nomination volume, submit-to-fund ratios, and funded volume.. Provided clear, data-driven insights during 1:1 coaching sessions, allowing for more objective and effective performance management.. 
------------------------------------
...
--- Concept: Sales Operations & Process ---
  ...
  1. Top Team Performance and Promotion Opportunity (from traditional_lending_subject_matter_expert_sme.org)
     ...
     Evidence: Applied data-driven coaching and sales enablement systems to drive the performance of the assigned 15-person sales team.. Led the highest-performing of the six teams in the program, resulting in being one of only two SMEs (out of six) offered a promotion to a full-time Account Executive role upon the program's conclusion.. 
--------------------------------------------------
  2. Strategic Sales Coaching (from traditional_lending_subject_matter_expert_sme.org)
     Relevance Score: 0.56
     Evidence: Coached reps on how to pivot from a failed traditional loan analysis to a high-margin product sale by translating complex financial rejection reasons into clear, effective sales talk tracks.. Enabled reps to save deals that would have otherwise been lost, preserving revenue and improving sales floor morale.. 
--------------------------------------------------
  3. Channel Partner Development (from business_relationship_manager.org)
...
def parse_accomplishments_from_file(file_path):
    ...
    accomplishment_start_regex = re.compile(r'^\*{3}\s+ACCOMPLISHMENT:\s*(.*)')
    star_regex = re.compile(r'^\s*:(SITUATION|TASK|ACTION|RESULT):\s*(.*)', re.IGNORECASE)
    # --- CORRECTED REGEX: Looks for the :SKILLS: property regardless of indentation ---
    skills_regex = re.compile(r'^\s*:SKILLS:\s*(.*)', re.IGNORECASE)
    skill_id_regex = re.compile(r'\[\[id:([\w-]+)\]\]')
    ...
    for line in content:
        ...
        if in_accomplishment_block and current_accomplishment:
            skills_match = skills_regex.match(line)
            if skills_match:
                skill_line = skills_match.group(1)
                ids = skill_id_regex.findall(skill_line)
